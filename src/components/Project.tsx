	...
	return (
		<div className="lg:w-80 lg:p-3  lg:flex lg:flex-col pro-main lg:rounded-2xl lg:mt-20 hvr-grow w-72 flex flex-col justify-items-center rounded-2xl justify-center p-3 text-sm">
			...
			<div className=" flex flex-col gap-5 mt-5  items-start p-2">
				<h1 className="text-2xl font-bold text-gray-300 items-start text-left">{title}</h1>
				<h6 className=" text-gray-400  flex items-start w-auto	text-left">{des}</h6>
			</div>
			<div className="lg:w-72 lg:mt-5 w-full mt-5 overflow-hidden">
				<div className="lg:grid lg:grid-cols-3 lg:font-thin lg:font-sans lg:opacity-40 grid grid-cols-3 opacity-40 font-sm font-sans grid-rows-3">
					{lang?.map((item: boolean | ReactChild | ReactFragment | ReactPortal) => (
						<span>{item}</span>
					))}
				</div>
			</div>
